...
Generating a Collection Occasion through the Online Measures Self Service System (OMSSS)
----------------------------------------------------------------------------------------
Outcome Measures can be collected from the client online through `OMSSS <https://docs.omsss.online>`_ instead
of using paper based collection followed by data entry.

The PMHC MDS has been integrated with OMSSS.

You can generate a link or QR code to send to the client by following these steps:

1. Navigate to :ref:`view-collection-occasion`
2. Check that the collection occasion is not already showing
3. Click the **Generate Self Service Link** tab
**TODO FILL IN REST OF INSTRUCTIONS**

Data Security and Privacy while using OMSSS and the PMHC MDS
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Please refer to `Data Security and Privacy <http://docs.omsss.online/en/latest/data-security-privacy.html>`_
for a general overview of data security and privacy in OMSSS including what
information is passed between OMSSS and the system integrating with it and how
long information is stored for.

The above documentation explains that the OMSSS system has been designed so that
it does not need to store any information that can identify the client. The
only required information associated with the outcome measure data is a unique
number, which is automatically generated by the OMSSS system. This number is
returned to the PMHC MDS, which is then associated with the client's record.
Therefore, when used in the context of the PMHC MDS, this ensures that no
identifying information is stored in OMSSS.

The above documentation also explains that a system that is integrated with OMSSS
can provide additional information when the measure is created. The PMHC MDS
integration with OMSSS uses a token containing an internal ID
for the organisation, episode, collection reason and the measure type. The
internal IDs only have meaning within the PMHC MDS data set,
and are signed for additional security.

When using the PMHC MDS - OMSSS integration the results of the client's outcome
measure may be sent to their health care provider, as well as being stored in
PMHC MDS. After the data is stored in the PMHC MDS its use is subject to the
same `client consent <https://docs.pmhc-mds.com/projects/data-specification/en/v2/data-model-and-specifications.html#episode-client-consent-to-anonymised-data>`_ as any
